const robotPath = function (room, src, dst) {
  ...
  const move = (m , n , candi , step) =>{
    ...
    //dfs 로 완전 탐색, 모든 알고리즘 문제에서 느끼길 재귀를 작성하는 것 자체는 복잡하지 않다. 컨셉을 떠올리는것에 익숙해져야 할듯...
    move(m,n,[row+1,col], step+1)
    move(m,n,[row-1,col], step+1)
    move(m,n,[row,col-1], step+1)
    move(m,n,[row,col+1], step+1)
  }

  move(room.length , room[0].length , src, 1)
